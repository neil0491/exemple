<template>
  <div ref="self" class="scroll-picker-container">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function setActiveOptionHeight(columns) {
      if (columns?.length > 0) {
    ...
    function getContent(event) {
      const index = Number(event.target.id.replace("column-", ""))
    ...
    function setSelections(selections) {
      ...
      const newSelections = props.options.map((rows, columnIndex) => {
        ...
        const rowIndex = rows.findIndex((row) => row.year === selection)
    ...
    function getValue(option) {
      return typeof option === "string" ? option : option.year
    }
    ...
    function filterColumnsContent(columns) {
      return columns.reduce((columns, column) => {
        if (column.overlay) {
    ...
    watch(
      ...
      async () => {
        nextTick(async () => {
          await nextTick(() => {
            setActiveOptionHeight(state.columns)
...
.pad-top-overlay-custom {
  height: 100%;
  background: linear-gradient(to top, rgba(24, 24, 24, 0), rgba(24, 24, 24, 1));
}
.pad-bottom-overlay {
  bottom: 0px;
}
